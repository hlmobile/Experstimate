...
-(void)prepareLayout
{
    CGFloat cellHeight = self.collectionView.frame.size.height / 5;
    [super prepareLayout];
    self.itemSize=CGSizeMake(self.collectionView.frame.size.width,self.pageHeight);
    self.minimumLineSpacing=-1*(self.itemSize.height-cellHeight);
...
-(CGSize)collectionViewContentSize{
    CGFloat cellHeight = self.collectionView.frame.size.height / 5;
    CGFloat contentHeight=cellHeight*([self.collectionView numberOfItemsInSection:0]-1)+self.collectionView.frame.size.height;//self.pageHeight;
