...
#define DEGREE_TO_RADIAN(x) (M_PI * x / 180.0)
...
- (void)hideAfterDelay
{
	[self performSelector:@selector(hide) withObject:nil afterDelay:1.6];
}
...
- (void)setProperRotation:(BOOL)animated
{
	...
	if (orientation == UIDeviceOrientationPortraitUpsideDown)
		self.transform = CGAffineTransformRotate(CGAffineTransformIdentity, DEGREE_TO_RADIAN(180));	
	
	else if (orientation == UIDeviceOrientationLandscapeLeft)
		self.transform = CGAffineTransformRotate(CGAffineTransformIdentity, DEGREE_TO_RADIAN(90));	
